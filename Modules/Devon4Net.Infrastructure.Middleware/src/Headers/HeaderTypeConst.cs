namespace Devon4Net.Infrastructure.Middleware.Headers
{
    public static class CustomMiddlewareHeaderTypeConst
    {
        public const string AccessControlExposeHeader = "Access-Control-Expose-Headers";
        
        public const string StrictTransportSecurityHeader = "Strict-Transport-Security";

        public const string XFrameOptionsHeader = "X-Frame-Options";

        public const string XssProtectionHeader = "X-XSS-Protection";

        public const string XContentTypeOptionsHeader = "X-Content-Type-Options";

        public const string ContentSecurityPolicyHeader = "Content-Security-Policy";

        public const string PermittedCrossDomainPoliciesHeader = "X-Permitted-Cross-Domain-Policies";

        public const string ReferrerPolicyHeader = "Referrer-Policy";
        
        public const string ServerHeader = "Server";

        public const string AspNetVersion = "X-AspNet-Version";

        public const string XPowered = "X-Powered-By";
    }

}
