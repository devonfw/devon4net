...
namespace fyiReporting.RDL
{
	...
	internal class ChartBar: ChartBase
	{
			...
			using (Graphics g = Graphics.FromImage(_mf))
			{
				...
				if (vaSize.Width > 0)	// If we made room for the axis - we need to draw it
					DrawValueAxis(rpt, g, min, max, 
						new System.DrawingCore.Rectangle(Layout.LeftMargin, _bm.Height-Layout.BottomMargin, _bm.Width - Layout.LeftMargin - Layout.RightMargin, vaSize.Height), Layout.TopMargin, _bm.Height - Layout.BottomMargin);

				// Draw Category Axis
				if (caSize.Height > 0)
					DrawCategoryAxis(rpt, g,  
						new System.DrawingCore.Rectangle(Layout.LeftMargin - caSize.Width, Layout.TopMargin, caSize.Width, _bm.Height - Layout.TopMargin - Layout.BottomMargin));
			...
			for (int iRow=1; iRow <= CategoryCount; iRow++)
			{
				...
				for (int iCol=1; iCol <= SeriesCount; iCol++)
				{
					...
					DrawColumnBar(rpt, g, GetSeriesBrush(rpt, iRow, iCol), 
						new System.DrawingCore.Rectangle(Layout.PlotArea.Left, barLoc, x, heightBar), iRow, iCol);
                    //Add a metafilecomment to use as a tooltip GJL 26092008
                    if (_showToolTips)
                    {
		...
		protected void DrawValueAxis(Report rpt, Graphics g, double min, double max, System.DrawingCore.Rectangle rect, int plotTop, int plotBottom)
		{
