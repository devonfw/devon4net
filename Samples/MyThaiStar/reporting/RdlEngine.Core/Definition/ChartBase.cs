...
namespace fyiReporting.RDL
{
	...
	//		_bm.Save(stream, im);
		...
		internal System.DrawingCore.Imaging.Metafile Image(Report rpt) // GJL 14082008 Using Vector Graphics
		{
			if (_bm == null)
				Draw(rpt);
		...
		{
			System.DrawingCore.Rectangle rect = new  System.DrawingCore.Rectangle(0, 0, Layout.Width, Layout.Height);
		...
		protected System.DrawingCore.Rectangle DrawLegend(Report rpt, Graphics g, bool bMarker, bool bBeforePlotDrawn)
		{
			Legend l = _ChartDefn.Legend;
			if (l == null)
				return System.DrawingCore.Rectangle.Empty;
			if (!l.Visible)
				return System.DrawingCore.Rectangle.Empty;
			...
			System.DrawingCore.Rectangle rRect;
			Style s = l.Style;
			try		// no matter what we want to dispose of the graphic resources
			{
				...
				rRect = new System.DrawingCore.Rectangle(x-1, y-1, totalWidth+2, totalHeight+2);
				if (s != null)
				{
				...
				for (int iCol=1; iCol <= SeriesCount; iCol++)
				{
                    ...
                    cm = ChartMarkerEnum.None;
                        ...
                        if ((bMarker || isLine) || (this.ChartDefn.Type == ChartTypeEnum.Scatter && t == typeof(System.DrawingCore.Drawing2D.HatchBrush)))
					...
					{
						case LegendLayoutEnum.Row:
							rect = new System.DrawingCore.Rectangle(x + boxSize + boxSize/2, y, totalItemWidth - boxSize - boxSize/2, h);
                                if (c != "") //14052008WRP to cater for empty strings in the legend
                                {
							g.DrawString(c, drawFont, drawBrush, rect, drawFormat);

                                    if ((cm != ChartMarkerEnum.None || this.ChartDefn.Type == ChartTypeEnum.Scatter) && (t == typeof(System.DrawingCore.Drawing2D.HatchBrush))) //GJL 110208 - Don't draw pattern for lines or Bubbles
                                    {
                                        System.DrawingCore.Drawing2D.HatchBrush hb = (System.DrawingCore.Drawing2D.HatchBrush)GetSeriesBrush(rpt, 1, iCol);
							...
							break;
						case LegendLayoutEnum.Table:
                            rect = new System.DrawingCore.Rectangle(x + boxSize + boxSize / 2, y, maxTextWidth, h);
                            g.DrawString(c, drawFont, drawBrush, rect, drawFormat);

                                if (cm != ChartMarkerEnum.None && (t == typeof(System.DrawingCore.Drawing2D.HatchBrush))) //GJL 110208 - Don't draw pattern for lines
                                {
                                    System.DrawingCore.Drawing2D.HatchBrush hb = (System.DrawingCore.Drawing2D.HatchBrush)GetSeriesBrush(rpt, 1, iCol);
                                    b = new SolidBrush(hb.ForegroundColor);
		...
		protected void DrawPlotAreaStyle(Report rpt, Graphics g, System.DrawingCore.Rectangle crect)
		{
			if (_ChartDefn.PlotArea == null || _ChartDefn.PlotArea.Style == null)
				return;
			System.DrawingCore.Rectangle rect = Layout.PlotArea;
	...
	//				g.Clip = saver;
		...
		protected void DrawTitle(Report rpt, Graphics g, Title t, System.DrawingCore.Rectangle rect)
		{
			if (t == null)
				return;
        ...
        protected void DrawCategoryLabel(Report rpt, Graphics g, string t, Style a, System.DrawingCore.Rectangle rect)
        {
...
// 20022008 AJM GJL
		...
		{
			DrawDataPoint(rpt, g, p, System.DrawingCore.Rectangle.Empty, row, col);
		}

		protected void DrawDataPoint(Report rpt, Graphics g, System.DrawingCore.Rectangle rect, int row, int col)
		{
			DrawDataPoint(rpt, g, Point.Empty, rect, row, col);
		}

		void DrawDataPoint(Report rpt, Graphics g, Point p, System.DrawingCore.Rectangle rect, int row, int col)
		{
			MatrixCellEntry mce = _DataDefn[row, col];
			if (mce == null)
				return;					// Not sure what this really means TODO:
			...
			{
				if (rect == System.DrawingCore.Rectangle.Empty)
				{
					Size size = Style.MeasureStringDefaults(rpt, g, v, tc, lrow, int.MaxValue);
					rect = new System.DrawingCore.Rectangle(p, size);
				}
...
// 20022008 AJM GJL
		...
		protected void AdjustMargins(System.DrawingCore.Rectangle legendRect, Report rpt, Graphics g)
		{
