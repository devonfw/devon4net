...
namespace Devon4Net.Infrastructure.AWS.CDK.Resources.Handlers.S3
{
    public class AwsCdkS3Handler : AwsCdkBaseHandler, IAwsCdkS3Handler
    {
        ...
        public IBucket AddLifeCycleRuleToExistingBucket(string identification, string bucketName, List<ILifecycleRule> lifecycleRulelist)
        {
            var bucket = LocateFromName(identification, bucketName);

            if (bucket == null)
            {
                throw new ArgumentException($"The S3 bucket {identification}|{bucketName} not found. Review the S3 identification and the name");
            }

            if (lifecycleRulelist?.Any() != true) return bucket;

            foreach (var lifecycleRule in lifecycleRulelist)
{
                ((Bucket)bucket).AddLifecycleRule(lifecycleRule);
            }

            return bucket;
        }

        public void AddLifeCycleRuleToExistingBucket(ref IBucket bucket, List<ILifecycleRule> lifecycleRulelist)
        {
            if (lifecycleRulelist?.Any() != true) return;

            if (bucket == null)
            {
                throw new ArgumentException($"The provided S3 bucket {nameof(bucket)} not found. Review the S3 identification and the name");
            }
