            ...
            foreach (var databaseOption in CdkOptions.Databases)
            {
                ...
                switch (databaseOption.DatabaseType?.ToLower())
                {
                    case "sqlserver":
                        ...
                        var monitoringRole = (!string.IsNullOrEmpty(databaseOption.MonitoringRoleId)) ? LocateRole(databaseOption.MonitoringRoleId, $"Role with id {databaseOption.MonitoringRoleId} not found in the JSON") : null;
