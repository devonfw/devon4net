    ...
    public class AopControllerAttribute : ActionFilterAttribute
    {
        ...
        public override void OnActionExecuted(ActionExecutedContext context)
        {
            try
            {
                ...
                if (context?.Result! is Microsoft.AspNetCore.Mvc.ObjectResult result) LogEvent("OnActionExecuted", result);
        ...
        public override void OnResultExecuted(ResultExecutedContext context)
        {
            try
            {
                if (context.Result is Microsoft.AspNetCore.Mvc.ObjectResult result) LogEvent("OnResultExecuted", result);
        ...
        public override async Task OnResultExecutionAsync(ResultExecutingContext context, ResultExecutionDelegate next)
        {
            try
            {
                if (context?.Result! is Microsoft.AspNetCore.Mvc.ObjectResult result) LogEvent("OnResultExecutionAsync", result);
                if (context != null) await base.OnResultExecutionAsync(context, next).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                Devon4NetLogger.Error(ex);
