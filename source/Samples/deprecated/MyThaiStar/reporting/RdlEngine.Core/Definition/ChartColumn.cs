...
namespace fyiReporting.RDL
{
	...
	internal class ChartColumn: ChartBase
	{
            ...
            using(Graphics g = Graphics.FromImage(_mf))
			{
				...
				if (vaSize.Width > 0)	// If we made room for the axis - we need to draw it
					DrawValueAxis(rpt, g, min, max, 
						new System.DrawingCore.Rectangle(Layout.LeftMargin - vaSize.Width, Layout.TopMargin, vaSize.Width, _bm.Height - Layout.TopMargin - Layout.BottomMargin), Layout.LeftMargin, Layout.Width - Layout.RightMargin,out incr,out intervalCount);
                ...
                //Secong value axis            
                if (Show2ndY)
                    Draw2ndValueAxis(rpt, g, min, max, new System.DrawingCore.Rectangle(Layout.LeftMargin + Layout.PlotArea.Width, Layout.TopMargin, vaSize2.Width, _bm.Height - Layout.TopMargin - Layout.BottomMargin), Layout.LeftMargin, Layout.Width - Layout.RightMargin, incr, intervalCount, ref ScaleFactor);
                         
				// Draw Category Axis
				if (caSize.Height > 0)
                    // 090508ajm passing chart bounds in
					DrawCategoryAxis(rpt, g,
                        new System.DrawingCore.Rectangle(Layout.LeftMargin, _bm.Height - Layout.BottomMargin, Layout.PlotArea.Width, caSize.Height), Layout.TopMargin, caSize.Width);
			...
			for (int iRow = 1; iRow <= CategoryCount; iRow++)
			{
				...
				for (int iCol = 1; iCol <= SeriesCount; iCol++)
				{
					...
					if (saveP == Point.Empty)
						rect = new System.DrawingCore.Rectangle(p, new Size(widthBar,h));
					else
						rect = new System.DrawingCore.Rectangle(p, new Size(widthBar, saveP.Y - p.Y));
                        ...
                        if (GetPlotType(rpt, iCol, iRow).ToUpper() != "LINE")
                        {
                            if (z == 0)
                            {
                                ...
                                DrawColumnBar(rpt, g, GetSeriesBrush(rpt, iRow, iCol),
                                    new System.DrawingCore.Rectangle(barLoc, Layout.PlotArea.Top + (maxBarHeight - h), widthBar, h), iRow, iCol);
                        ...
                        else //This is a line type plot
                        {
                            if (z == 1)
                            {
                                ...
                                if (DrawPoint)
                                {
                                    ...
                                    System.DrawingCore.Rectangle r = new System.DrawingCore.Rectangle(lineLoc, Layout.PlotArea.Top + (maxBarHeight - h), widthBar, h);
        ...
        {
             if (b.GetType() == typeof(System.DrawingCore.Drawing2D.HatchBrush))
             {
                System.DrawingCore.Drawing2D.HatchBrush hb = ( System.DrawingCore.Drawing2D.HatchBrush) b;
        ...
        protected void DrawCategoryAxis(Report rpt, Graphics g, System.DrawingCore.Rectangle rect, int plotTop, int maxWidth)
		{
			if (this.ChartDefn.CategoryAxis == null)
				return;
			Axis a = this.ChartDefn.CategoryAxis.Axis;
            ...
            for (int iRow=1; iRow <= CategoryCount; iRow++)
			{
				...
				{
					System.DrawingCore.Rectangle drawRect;
                    Size size = Size.Empty;
					...
					{	// Area chart - value is centered under the tick mark
						drawRect = 
								new System.DrawingCore.Rectangle(drawLoc - size.Width/2, rect.Top+tickSize, size.Width, size.Height);
					}
					else	// Column/Line charts are just centered in the region.
						drawRect = new System.DrawingCore.Rectangle(drawLoc, rect.Top+tickSize, drawWidth, rect.Height-tSize.Height);
                ...
                if (a.Month && date && a.Visible)
                {
                    ...
                    if (CurrentDate.Month != OldDate.Month)
                    {
                        ...
                        DrawCategoryLabel(rpt, g, MonthString, a.Style, new System.DrawingCore.Rectangle(catlabelLoc, rect.Top - (lSize.Height - 25), lSize.Width, lSize.Height));
		...
		void DrawColumnBar(Report rpt, Graphics g, Brush brush, System.DrawingCore.Rectangle rect, int iRow, int iCol)
		{
            ...
            rect = new System.DrawingCore.Rectangle(rect.Left + 2, rect.Top, rect.Width -3, rect.Height);
		...
		protected void DrawValueAxis(Report rpt, Graphics g, double min, double max,
                        System.DrawingCore.Rectangle rect, int plotLeft, int plotRight, out double incr,out int intervalCount)
		{
            incr = 0;
            intervalCount = 0;
        ...
        protected void Draw2ndValueAxis(Report rpt, Graphics g, double min, double max,
                        System.DrawingCore.Rectangle rect, int plotLeft, int plotRight,double incr,int intervalCount,ref double ScaleFactor)
        {           
           ...
           // rect.Width = (int)g.VisibleClipBounds.Width - rect.Left + 20;
            ...
            DrawTitle(rpt, g, a.Title2, new System.DrawingCore.Rectangle((int)rect.Right - tSize.Width, rect.Top, tSize.Width, rect.Height));
            ...
            for (int i = 0; i < _gridIncrs + 1; i++)
            {
                ...
                else if (s != null)
                {
                    Size size = s.MeasureString(rpt, g, v, TypeCode.Double, null, int.MaxValue);
                    System.DrawingCore.Rectangle vRect =
                        new System.DrawingCore.Rectangle(rect.Left - (int)(tSize.Width * .5), rect.Top + rect.Height - h - size.Height / 2, rect.Width - tSize.Width, size.Height);
                    s.DrawString(rpt, g, v, TypeCode.Double, null, vRect);
                }
                else
                {
                    Size size = Style.MeasureStringDefaults(rpt, g, v, TypeCode.Double, null, int.MaxValue);
                    System.DrawingCore.Rectangle vRect =
                        new System.DrawingCore.Rectangle(rect.Left - (int)(tSize.Width * .5), rect.Top + rect.Height - h - size.Height / 2, rect.Width - tSize.Width * 2, size.Height);
                    Style.DrawStringDefaults(g, v, vRect);
                }          
