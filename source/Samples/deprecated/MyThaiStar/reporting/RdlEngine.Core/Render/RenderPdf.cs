...
namespace fyiReporting.RDL
{
    ...
    internal class RenderPdf : IPresent
    {
        ...
        /// </summary>
        /// <returns></returns>
        private void iAddLine(float x, float y, float x2, float y2, float width, System.DrawingCore.Color c, BorderStyleEnum ls)
        {
            // Get the line color			
        ...
        private void iAddCurve(float X1, float Y1, float X2, float Y2, float X3, float Y3, float X4, float Y4, StyleInfo si, string url)
        {
            ...
            if (!si.BackgroundColor.IsEmpty)
            {
                cb.SetRgbColorStroke(si.BackgroundColor.R, si.BackgroundColor.G, si.BackgroundColor.B);
            }
        ...
        private float MeasureStringBlank(Graphics g, System.DrawingCore.Font drawFont, StringFormat drawFormat)
        {
