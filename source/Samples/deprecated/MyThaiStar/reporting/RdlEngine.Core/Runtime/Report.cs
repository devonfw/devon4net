...
namespace fyiReporting.RDL
{
	...
	public class Report
	{
		...
		public void RunRender(IStreamGen sg, OutputPresentationType type, Pages pgs)
		{
			RunRender(sg, type, "",pgs);
		}
		...
		private void RunRenderMht(IStreamGen sg, Pages pgs)
		{
			...
			try
			{
				...
				RunRender(temp, OutputPresentationType.HTML,  pgs);
        ...
        public void SetEmbebedImageItem(string imageItem, string imageBaseValue)
        {
            var item = (EmbeddedImage)_Report.LUEmbeddedImages[imageItem];

            if (item == null) return;
            item.ImageData = imageBaseValue;
